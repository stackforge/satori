...
def run(target, config=None, interactive=False):
    ...
    if utils.is_valid_ip_address(target):
        ...
        ipaddress = six.text_type(dns.resolve_hostname(hostname))
        #TODO(sam): Use ipaddress.ip_address.is_global
        #                   "               .is_private
        #                   "               .is_unspecified
        #                   "               .is_multicast
        #       To determine address "type"
        if not ipaddress_module.ip_address(ipaddress).is_loopback:
            try:
                ...
                resources[identifier]['params'] = domain_info
    ...
    if config.get('username'):
        ...
        if server:
            ...
            params = {}
            host['params'] = params
            params['uri'] = [l['href'] for l in server.links
                             if l['rel'] == 'self'][0]
            params['name'] = server.name
            params['id'] = server.id
            params['addresses'] = server.addresses
            host['key'] = params['uri']
            resources[host['key']] = host
            found['host-key'] = host['key']
        else:
            found['host'] = host
    else:
        found['host'] = host
    ...
    if config.get('system_info'):
        ...
        try:
            result = system_info_module.get_systeminfo(
                ipaddress, config, interactive=interactive)
