...
def get_systeminfo(ipaddress, config, interactive=False):
    ...
    """
    if (ipaddress in localhost.local_ips() or
            IPy.IP('127.0.0.0/8').overlaps(ipaddress) == 1):
        client = type("LocalHostDataPlaneClient", (),
                      {'execute': localhost.local_execute,
                       'platform_info': localhost.platform_info(),
                       'host': str(ipaddress),
                       'port': 0})()
    else:
        client = ssh.connect(host=ipaddress, username=config.host_username,
                             private_key=config.host_key, interactive=interactive)
        client.execute = client.remote_execute
