...
class TestArgParsing(utils.TestCase):
    ...
    @mock.patch('%s.open' % BUILTINS)
    def test_host_key_path_sets_host_key_contents(self, mock_open):
        args = sys.argv
        args.append('--host-key-path=foo.key')
        handle = mock.MagicMock()
        mock_open.return_value = handle
        handle.read.return_value = 'some data'

        config = shell.parse_args(args)
        self.assertEqual('some data', config.host_key)
